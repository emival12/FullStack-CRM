...
async def endpoint_update_object(request: Request, cursor=Depends(get_cursor)):
    data = await request.json()
...
async def endpoint_delete_object(request: Request, cursor=Depends(get_cursor)):
    data = await request.json()
    table_name = data.get("table")

    result = delete_object(cursor, table_name)  # raise 500, 404-INPUT_TABLE_NAME_NOT_FOUND
    return result

@router.post("/fields/delete")
async def endpoint_delete_field(request: Request, cursor=Depends(get_cursor)):
    data = await request.json()
    table_name = data.get("table")
    field_name = data.get("fieldName")

    result = delete_field(cursor, table_name, field_name) # raise 500, 404-INPUT_TABLE_NAME_NOT_FOUND
    return result
...
async def endpoint_create_field(request: Request, cursor=Depends(get_cursor)):
    data = await request.json()
    table_name = data.get("table")
    field_data = data.get("record")

    result = create_field(cursor, table_name, field_data) # raise 500
    return result
