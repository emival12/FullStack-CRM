...
from engines import formulaEngine, rollupEngine
...
def get_table_records(cursor, table_name: str, record_type_name: str) -> dict:
    ...
    obj_key = get_table_key_from_strings(table_name, record_type_name)
    fields = dict_fields.get(obj_key, None)
    if not fields:
        raise_server_exception(f"get_table_records: {obj_key} doesn't have fields")
...
def insert_record(cursor, db, table_name: str, record_type_name: str, record: dict, user_id: str) -> dict:
    """
        Insert a new record into the given table, evaluating formulas and triggers, then refreshing parent rollups.

        Args:
            cursor (MySQLCursor): Database cursor used to execute SQL queries.
            db (MySQLConnection): Database connection used to commit or rollback changes.
    ...
    if not fields:
        raise_server_exception(f"insert_record: no fields found for {obj_key}")
    ...
    complex_formula = formulaEngine.extract_formula_dependencies(map_field_by_type.formula_fields, already_extracted, fields_to_retrieve)
    ...
    record = formulaEngine.cast_record_types(record, fields)

    try:
        # Run BEFORE INSERT triggers, which may modify raw field values before formula evaluation
        record = triggerManager.run_triggers(cursor, get_triggers_folder(), table_name, "BEFORE", "INSERT", record)

        # Evaluate formulas after the trigger so they see the final field values
        record = formulaEngine.evaluate_all_formulas(cursor, fields, record, complex_formula)
        ...
        impacted_parents = rollupEngine.get_impacted_parents(cursor, table_name, record)
        if impacted_parents:
            rollupEngine.refresh_parents(cursor, db, impacted_parents, user_id)

        # Run AFTER INSERT triggers. AFTER Triggers can only modify other records or make new DML operations
        triggerManager.run_triggers(cursor, get_triggers_folder(), table_name, "AFTER", "INSERT", record)
...
def update_record(cursor, db, table_name: str, record_type_name: str, record_id: str, new_record: dict, user_id: str, map_object_primary_key_names: dict = None) -> dict:
    """
    ...
    fields = get_fields_definition(cursor, [(table_name, record_type_name)], is_visible=0).get(obj_key, [])
    if not fields:
        raise_server_exception(f"update_record: no fields found for {obj_key}")
    ...
    if primary_key_field is None:
        raise_server_exception(f"update_record: primary key cannot be None for table {table_name}")
    ...
    rollup_values = rollupEngine.calculate_record_rollups(cursor, table_name, primary_key_field, record_id, map_field_by_type.rollup_fields, rollup_map)
    ...
    try:
        ...
        record = triggerManager.run_triggers(cursor, get_triggers_folder(), table_name, "BEFORE", "UPDATE", record)
        ...
        # Propagate changes upward: refresh any parent rollup fields that depend on this record
        impacted_parents = rollupEngine.get_impacted_parents(cursor, table_name, record, old_record)
        if impacted_parents:
            rollupEngine.refresh_parents(cursor, db, impacted_parents, user_id)

        # Run AFTER UPDATE triggers. AFTER Triggers can only modify other records or make new DML operations
        triggerManager.run_triggers(cursor, get_triggers_folder(), table_name, "AFTER", "UPDATE", record)
        db.commit()
    except Exception as e:
        db.rollback()
        raise
...
def delete_record(cursor, db, table_name: str, record_type_name: str, record_id: str) -> dict:
    primary_key_field = get_primary_keys_from_multiple_objects(cursor, [table_name]).get(table_name)
    if primary_key_field is None:
        raise_server_exception(f"delete_record: primary key cannot be None for table {table_name}")
